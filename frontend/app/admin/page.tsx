    ...
    const clearSecurity = useCallback(
        async (ip?: string) => {
            ...
            try {
                ...
                showAlert(
                    ip ? `Cleared security state for ${ip}.` : "Cleared all security state.",
                    "success",
                );
                await loadSecurity(key);
            } catch {
            ...
            <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
                ...
                <section className="rounded-4xl border border-white/10 bg-white/92 p-6 text-slate-900 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 className="text-lg font-semibold text-slate-900">Security State</h2>
                            <p className="text-sm text-slate-500">
                                View and clear persisted blocks, suspicion scores, and rate-limit buckets.
                            </p>
                        </div>
                        <div className="flex gap-2">
                            <button
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                            <h3 className="text-sm font-semibold text-slate-900">Blocked IPs</h3>
                            <p className="mt-1 text-xs text-slate-500">{securityOverview.blocked.length} active</p>
                            <ul className="mt-3 max-h-56 space-y-2 overflow-y-auto text-xs">
                                {securityOverview.blocked.map((item) => (
                                    <li key={`blocked-${item.ip}`} className="rounded-xl border border-slate-100 bg-slate-50 p-2">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                            <h3 className="text-sm font-semibold text-slate-900">Suspicious IPs</h3>
                            <p className="mt-1 text-xs text-slate-500">{securityOverview.suspicious.length} tracked</p>
                            <ul className="mt-3 max-h-56 space-y-2 overflow-y-auto text-xs">
                                {securityOverview.suspicious.map((item) => (
                                    <li key={`suspicious-${item.ip}`} className="rounded-xl border border-slate-100 bg-slate-50 p-2">
                                        <p className="font-semibold text-slate-900">{item.ip}</p>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                            <h3 className="text-sm font-semibold text-slate-900">Rate-Limit Buckets</h3>
                            <p className="mt-1 text-xs text-slate-500">{securityOverview.rateLimited.length} active</p>
                            <ul className="mt-3 max-h-56 space-y-2 overflow-y-auto text-xs">
                                {securityOverview.rateLimited.map((item) => (
                                    <li key={`rate-${item.ip}-${item.method}-${item.path}`} className="rounded-xl border border-slate-100 bg-slate-50 p-2">
                                        <p className="font-semibold text-slate-900">{item.ip}</p>
                                        <p className="text-slate-500">{item.method} {item.path}</p>
