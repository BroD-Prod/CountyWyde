...
db.exec('CREATE INDEX IF NOT EXISTS idx_security_rate_limits_reset_at ON security_rate_limits(reset_at);');
db.exec('CREATE INDEX IF NOT EXISTS idx_security_blocks_blocked_until ON security_blocks(blocked_until);');
db.exec('CREATE INDEX IF NOT EXISTS idx_security_suspicion_reset_at ON security_suspicion(reset_at);');
...
function addSuspicion(req, delta, reason) {
    ...
    const resetAt = !row || row.resetAt <= now ? now + SCORE_WINDOW_MS : row.resetAt;
...
function clearSecurityForIp(ip) {
    ...
    const tx = db.transaction(() => {
        const a = db.prepare('DELETE FROM security_blocks WHERE ip = ?').run(normalizedIp).changes;
        const b = db.prepare('DELETE FROM security_suspicion WHERE ip = ?').run(normalizedIp).changes;
        const c = db.prepare('DELETE FROM security_rate_limits WHERE ip = ?').run(normalizedIp).changes;
        return a + b + c;
    });
