...
function getRegisteredCounties(state) {
    ...
    const rows = selectedState
        ...
        : db
            .prepare("SELECT DISTINCT county FROM accounts WHERE county IS NOT NULL AND TRIM(county) <> '' ORDER BY county ASC")
            .all();
...
function isRegisteredState(value) {
    ...
    const row = db
        .prepare('SELECT 1 FROM states WHERE LOWER(name) = LOWER(?) OR UPPER(abbreviation) = UPPER(?) LIMIT 1')
        .get(state, state);
