...
async function main() {
    ...
    const updated = pending.filter((item) => Array.isArray(item.embedding) && item.embedding.length > 0).length;
    const failed = pending.length - updated;

    await writeUploads(uploads);
    console.log(`Backfill complete. updated=${updated}, failed=${failed}, total=${uploads.length}`);
}
