...
async function main() {
  if (!process.env.MILVUS_ADDRESS && !process.env.GEMINI_API_KEY) {
    console.warn(
      "Warning: MILVUS_ADDRESS not set — defaulting to localhost:19530",
    );
  }
  ...
  if (withEmbeddings.length === 0) {
    console.log(
      "No chunks with embeddings found. Run backfill:embeddings first.",
    );
    return;
  }

  console.log(
    `Found ${withEmbeddings.length} embedded chunks out of ${uploads.length} total.`,
  );
  ...
  for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
    ...
    console.log(
      `  [${done}/${chunks.length}] inserted=${inserted} skipped=${skipped}`,
    );
  }
