...
async function createAccount(req, res) {
  ...
  res.end(
    JSON.stringify({
      error:
        "Direct signup is disabled. Request county access through the contact page for admin approval.",
    }),
  );
}
...
async function changePassword(req, res) {
  ...
  try {
    ...
    await db.prepare("UPDATE sessions SET expires_at = ? WHERE user_id = ?").run(
...
async function resetPasswordWithToken(req, res) {
  ...
  try {
    ...
    await db.prepare("DELETE FROM sessions WHERE user_id = ?").run(
      resetRecord.user_id,
    );
