...
const {
  ...
  chunkText,
  normalizeChunkText,
} = require("./uploadFilesControllers");
...
const VIDEO_TRANSCRIPTS_DIR = path.join(
  __dirname,
...
function resolveVideoExtension({ videoPath, contentType }) {
  ...
  const extFromType =
    MIME_EXTENSION_MAP[String(contentType || "").toLowerCase()] || "";
...
async function transcribeAudioWithWhisper(audioPath) {
  if (!WHISPER_MODEL_PATH) {
    throw new Error(
      "Missing WHISPER_MODEL_PATH for local whisper.cpp transcription",
    );
  }
  ...
  const outputPrefix = path.join(
    VIDEO_TRANSCRIPTS_DIR,
    `${path.basename(audioPath, path.extname(audioPath))}-${createId()}`,
  );
  ...
  return new Promise((resolve, reject) => {
    const args = [
      ...
      "-t",
      String(
        Number.isFinite(WHISPER_THREADS) && WHISPER_THREADS > 0
          ? WHISPER_THREADS
          : 4,
      ),
    ];
