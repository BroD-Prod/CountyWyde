module.exports = {
    ...
    name: 'create initial schema and seed data',
    up: async (db) => {
        await db.query(`
    ...
    `);
        ...
        await db.query("ALTER TABLE accounts ADD COLUMN IF NOT EXISTS county TEXT");
        await db.query(
            "UPDATE accounts SET county = 'Unknown County' WHERE county IS NULL OR TRIM(county) = ''",
        );

        await db.query("ALTER TABLE accounts ADD COLUMN IF NOT EXISTS state_id INTEGER");
        await db.query("ALTER TABLE accounts ADD COLUMN IF NOT EXISTS approved BOOLEAN DEFAULT FALSE");
        await db.query("UPDATE accounts SET approved = TRUE WHERE approved IS NULL");

        const defaultState = await db.query("SELECT id FROM states WHERE abbreviation = 'AL' LIMIT 1");
        const defaultStateId = defaultState.rows[0]?.id || 1;
        await db.query("UPDATE accounts SET state_id = ? WHERE state_id IS NULL", [defaultStateId]);
    },
};
